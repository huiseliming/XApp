...
void CVKRenderer::Init(GLFWwindow *main_window) {
...
void CVKRenderer::Render() {
  ...
  VK_SUCCEEDED(
      Device.waitForFences(1, &RenderFrame.InFlightFence, VK_TRUE, UINT64_MAX));
  auto ImageIndexResult = Device.acquireNextImageKHR(
      Swapchain, UINT64_MAX, RenderFrame.ImageAvailableSemaphore, nullptr);
  ...
  vk::ClearValue clear_color = vk::ClearColorValue{ClearColor.x, ClearColor.y,
  ...
  // PipelineLayout, 0, RenderFrame.DescriptorSet, nullptr);
...
void CVKRenderer::CreateInstance() {
  ...
  auto glfw_required_instance_extensions = glfwGetRequiredInstanceExtensions(&InstanceExtensionsCount);
  for (size_t i = 0; i < InstanceExtensionsCount; i++)
  {
      EnabledExtensions.push_back(glfw_required_instance_extensions[i]);
  }
  ...
  SPDLOG_ERROR("VulkanInstanceVersion : {}.{}.{}",
               VK_API_VERSION_MAJOR(VulkanInstanceVersion),
               VK_API_VERSION_MINOR(VulkanInstanceVersion),
               VK_API_VERSION_PATCH(VulkanInstanceVersion));
  {
    SPDLOG_ERROR("Support Extensions :");
    vk::ResultValue<std::vector<vk::ExtensionProperties>>
        SupportedExtensionsResult = vk::enumerateInstanceExtensionProperties();
  ...
  vk::ResultValue<std::vector<vk::LayerProperties>> SupportedLayersResult =
...
#endif
    vk::ResultValue<std::vector<vk::ExtensionProperties>>
        SupportedExtensionsResult =
            vk::enumerateInstanceExtensionProperties(SupportedLayers.back());
    VK_SUCCEEDED(SupportedExtensionsResult.result);
    for (auto &SupportedExtension : SupportedExtensionsResult.value) {
      SupportedExtensionSet.insert(SupportedExtension.extensionName);
#ifndef NDEBUG
  ...
  bool bIsPortabilityEnumerationSupport =
      IsExtensionSupported(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
...
#if VK_EXT_validation_features
  if (bEnableValidationLayers) {
    vk::ValidationFeatureEnableEXT ValidationFeatureEnableEXT[] = {
  ...
  vk::ResultValue<vk::Instance> InstanceResult =
  ...
  if (bUseDebugMessenger) {
    ...
    vk::ResultValue<vk::DebugUtilsMessengerEXT> DebugUtilsMessengerResult =
...
void CVKRenderer::CreateSurface() {
  VkSurfaceKHR main_window_surface;
  X_RUNTIME_ASSERT(VK_SUCCESS == glfwCreateWindowSurface(Instance, MainWindow, nullptr, &main_window_surface));
  Surface = main_window_surface;
}

void CVKRenderer::CreateDevice() {
  std::vector<int32> PhysicalDeviceScores;
  vk::ResultValue<std::vector<vk::PhysicalDevice>>
      AvailablePhysicalDevicesResult = Instance.enumeratePhysicalDevices();
  ...
  for (auto &AvailablePhysicalDevice : AvailablePhysicalDevicesResult.value) {
    ...
    vk::PhysicalDeviceProperties PhysicalDeviceProperties =
        AvailablePhysicalDevice.getProperties();
    SPDLOG_ERROR("    DeviceName : {}",
    ...
    std::vector<vk::QueueFamilyProperties> QueueFamilyProperties =
        AvailablePhysicalDevice.getQueueFamilyProperties();
    for (int32 i = 0; i < QueueFamilyProperties.size(); i++) {
      if (QueueFamilyProperties[i].queueFlags & vk::QueueFlagBits::eGraphics) {
        vk::ResultValue<uint32> SurfaceSupportResult =
            AvailablePhysicalDevice.getSurfaceSupportKHR(i, Surface);
        if (SurfaceSupportResult.result == vk::Result::eSuccess &&
            SurfaceSupportResult.value != 0) {
  ...
  X_RUNTIME_ASSERT(PhysicalDeviceIndex >= 0,
                   "NOT FIND SUPPORTED PHYSICAL DEVICE");
  PhysicalDevice = AvailablePhysicalDevicesResult.value[PhysicalDeviceIndex];

  SPDLOG_ERROR("Use PhysicalDevices : {}",
  ...
  for (uint32 i = 0; i < QueueFamilyProperties.size(); i++) {
    if (QueueFamilyProperties[i].queueFlags & vk::QueueFlagBits::eGraphics) {
      ...
      vk::ResultValue<uint32> PresentFamilyIndexResult =
          PhysicalDevice.getSurfaceSupportKHR(i, Surface);
      if (PresentFamilyIndexResult.result == vk::Result::eSuccess &&
          PresentFamilyIndexResult.value != 0) {
  ...
  DeviceCreateInfo.setQueueCreateInfoCount(
      GraphicsFamilyIndex != PresentFamilyIndex ? 2 : 1);
  ...
  vk::ResultValue<vk::Device> DeviceResult =
  ...
  PresentQueue = GraphicsFamilyIndex != PresentFamilyIndex
                     ? Device.getQueue(PresentFamilyIndex, 0)
                     : GraphicsQueue;
}
...
void CVKRenderer::CreateSwapchain() {
  vk::ResultValue<vk::SurfaceCapabilitiesKHR> SurfaceCapabilitiesResult =
      PhysicalDevice.getSurfaceCapabilitiesKHR(Surface);
  VK_SUCCEEDED(SurfaceCapabilitiesResult.result);
  SurfaceCapabilities = SurfaceCapabilitiesResult.value;

  vk::ResultValue<std::vector<vk::SurfaceFormatKHR>> SurfaceFormatsResult =
      PhysicalDevice.getSurfaceFormatsKHR(Surface);
  VK_SUCCEEDED(SurfaceFormatsResult.result);
  SurfaceFormats = SurfaceFormatsResult.value;

  vk::ResultValue<std::vector<vk::PresentModeKHR>> PresentModesResult =
  ...
  MinImageCount = std::min(SurfaceCapabilities.maxImageCount,
  ...
  vk::ResultValue<vk::SwapchainKHR> SwapchainResult =
      Device.createSwapchainKHR(SwapchainCreateInfo);
  ...
  vk::ResultValue<std::vector<vk::Image>> SwapchainImagesResult =
      Device.getSwapchainImagesKHR(Swapchain);
  VK_SUCCEEDED(SwapchainImagesResult.result);
  X_ASSERT(RenderFrames.size() == 0 ||
           RenderFrames.size() == SwapchainImagesResult.value.size());
  ...
  for (int32 i = 0; i < RenderFrames.size(); i++) {
    ...
    vk::ResultValue<vk::ImageView> SwapchainImageViewResult =
...
void CVKRenderer::CreateRenderPass() {
  ...
  ColorAttachmentDescs.back().finalLayout =
...
void CVKRenderer::CreateFramebuffers() {
  for (size_t i = 0; i < RenderFrames.size(); i++) {
    std::vector<vk::ImageView> Attachments = {
        RenderFrames[i].SwapchainImageView};
...
void CVKRenderer::CreateSyncObjects() {
  for (size_t i = 0; i < RenderFrames.size(); i++) {
    ...
    auto ImageAvailableSemaphoreResult =
        Device.createSemaphore(SemaphoreCreateInfo);
    VK_SUCCEEDED(ImageAvailableSemaphoreResult.result);
    RenderFrames[i].ImageAvailableSemaphore =
        ImageAvailableSemaphoreResult.value;

    auto RenderFinishedSemaphoreResult =
        Device.createSemaphore(SemaphoreCreateInfo);
    VK_SUCCEEDED(RenderFinishedSemaphoreResult.result);
    RenderFrames[i].RenderFinishedSemaphore =
        RenderFinishedSemaphoreResult.value;
...
void CVKRenderer::DestroyRenderPass() { Device.destroyRenderPass(RenderPass); }
...
void CVKRenderer::DestroyDevice() { Device.destroy(); }

void CVKRenderer::DestroySurface() { Instance.destroySurfaceKHR(Surface); }
...
VkBool32 CVKRenderer::DebugUtilsMessengerCallback(
  ...
  ReportFormatter
      ...
      << ":\n";
  ReportFormatter << std::string("\t") << "messageIDName   = <"
                  << pCallbackData->pMessageIdName << ">\n";
  ReportFormatter << std::string("\t")
                  << "messageIdNumber = " << pCallbackData->messageIdNumber
                  << "\n";
  ReportFormatter << std::string("\t") << "message         = <"
                  << pCallbackData->pMessage << ">\n";
  if (0 < pCallbackData->queueLabelCount) {
    ReportFormatter << std::string("\t") << "Queue Labels:\n";
    for (uint32_t i = 0; i < pCallbackData->queueLabelCount; i++) {
      ReportFormatter << std::string("\t\t") << "labelName = <"
  ...
  if (0 < pCallbackData->objectCount) {
    ...
    for (uint32_t i = 0; i < pCallbackData->objectCount; i++) {
      ...
      ReportFormatter << std::string("\t\t\t") << "objectType   = "
                      << vk::to_string(static_cast<vk::ObjectType>(
                             pCallbackData->pObjects[i].objectType))
                      << "\n";
      ReportFormatter << std::string("\t\t\t") << "objectHandle = "
                      << pCallbackData->pObjects[i].objectHandle << "\n";
      if (pCallbackData->pObjects[i].pObjectName) {
        ReportFormatter << std::string("\t\t\t") << "objectName   = <"
...
//
// IShader* CVKRenderer::CreateShader(EShaderType shader_type, const char*
// source, const char* entry_point) {
//  std::string shader_code = Utils::LoadCodeFromFile(source);
//  const glslang_input_t input = {.language = GLSLANG_SOURCE_GLSL,
//                                 .stage =
//                                 GetGLSLangStageShaderType(shader_type),
//                                 .client = GLSLANG_CLIENT_VULKAN,
//                                 .client_version = GLSLANG_TARGET_VULKAN_1_1,
//                                 .target_language = GLSLANG_TARGET_SPV,
//                                 .target_language_version =
//                                 GLSLANG_TARGET_SPV_1_3, .code =
//                                 shader_code.c_str(), .default_version = 460,
...
//          if (glslang_program_link(glslang_program, GLSLANG_MSG_SPV_RULES_BIT
//          | GLSLANG_MSG_VULKAN_RULES_BIT)) {
//            glslang_program_SPIRV_generate(glslang_program, input.stage);
//            if (auto messages =
//            glslang_program_SPIRV_get_messages(glslang_program)) {
//              SPDLOG_WARN("glslang_program_SPIRV_get_messages : {}",
//              messages);
//            }
//            vk::ShaderModuleCreateInfo shader_module_create_info;
//            shader_module_create_info.codeSize =
//            glslang_program_SPIRV_get_size(glslang_program) * sizeof(unsigned
//            int); shader_module_create_info.pCode =
//            glslang_program_SPIRV_get_ptr(glslang_program); auto
//            ShaderModuleResult =
...
//          } else {
//            LastErrorCode = -1;
//            LastErrorMessage = fmt::format("glslang_program_link : {} ({})!",
...
//        } else {
//          LastErrorCode = -1;
//          LastErrorMessage = fmt::format("glslang_program_create : {} ({})!",
//          glslang_shader_get_info_log(glslang_shader),
//                                         glslang_shader_get_info_debug_log(glslang_shader));
//        }
//      } else {
//        LastErrorCode = -1;
//        LastErrorMessage = fmt::format("glslang_shader_parse : {} ({})!",
//        glslang_shader_get_info_log(glslang_shader),
//                                       glslang_shader_get_info_debug_log(glslang_shader));
//      }
//    } else {
//      LastErrorCode = -1;
//      LastErrorMessage = fmt::format("glslang_shader_preprocess : {} ({})!",
//      glslang_shader_get_info_log(glslang_shader),
//                                     glslang_shader_get_info_debug_log(glslang_shader));
//    }
...
//  } else {
//    LastErrorCode = -1;
//    LastErrorMessage = fmt::format("glslang_shader_create : {} ({})!",
//    glslang_shader_get_info_log(glslang_shader),
//                                   glslang_shader_get_info_debug_log(glslang_shader));
//  }
//  return nullptr;
