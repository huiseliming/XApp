...
protected:
  ...
  CVKImGuiLayer *GetVKImGuiLayer() {
    return static_cast<CVKImGuiLayer *>(ImGuiLayer);
  }
...
protected:
  ...
  FORCEINLINE void
  SubmitCommandBufferWaitIdle(vk::CommandBuffer command_buffer) {
  ...
  FORCEINLINE SCreateBufferResult
  CreateBuffer(vk::BufferCreateInfo buffer_ci,
    ...
    X_ASSERT(VK_SUCCESS ==
             vmaCreateBuffer(
                 MemoryAllocator, &buffer_ci.operator VkBufferCreateInfo &(),
                 &allocation_ci, &result.Buffer, &result.Allocation, nullptr));
    return result;
  }

  FORCEINLINE SCreateImageResult
  CreateImage(vk::ImageCreateInfo image_ci,
              VmaAllocationCreateFlags allocation_create_flags,
              VmaMemoryUsage memory_usage = VMA_MEMORY_USAGE_AUTO) {
    VmaAllocationCreateInfo allocation_ci = {};
    allocation_ci.flags = allocation_create_flags;
    ...
    X_ASSERT(VK_SUCCESS ==
             vmaCreateImage(
                 MemoryAllocator, &image_ci.operator VkImageCreateInfo &(),
                 &allocation_ci, &result.Image, &result.Allocation, nullptr));
  ...
  FORCEINLINE SCreateBufferResult CreateTransferSrc(vk::DeviceSize size) {
    ...
    return CreateBuffer(buffer_ci,
                        VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT |
                            VMA_ALLOCATION_CREATE_MAPPED_BIT);
  }

  FORCEINLINE void CopyBufferToImage(vk::Buffer buffer, vk::Image image,
    ...
    command_buffer.copyBufferToImage(
        buffer, image, vk::ImageLayout::eTransferDstOptimal, buffer_image_copy);
    SubmitCommandBufferWaitIdle(command_buffer);
  }
  FORCEINLINE void TransitionImageLayout(vk::Image image, vk::Format format,
...
protected:
  ...
  FORCEINLINE uint32
  FindMemoryTypeIndex(uint32 InMemoryTypeBits /* SupportetMemoryIndices */,
...
vk::Extent2D CVKRenderer::ChooseSwapchainExtent() {
  ...
  vk::Extent2D SwapchainExtent{static_cast<uint32>(Width),
...
uint32 CVKRenderer::FindMemoryTypeIndex(
    uint32 InMemoryTypeBits /* SupportetMemoryIndices */,
    vk::MemoryPropertyFlags InRequestedMemoryPropertyFlags) {
  ...
  for (uint32 i = 0; i < MemoryProperties.memoryTypeCount; i++) {
    if ((InMemoryTypeBits & (1 << i)) &&
        (MemoryProperties.memoryTypes[i].propertyFlags &
         InRequestedMemoryPropertyFlags) == InRequestedMemoryPropertyFlags) {
      return i;
    }
  }
