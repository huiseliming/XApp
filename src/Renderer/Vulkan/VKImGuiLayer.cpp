...
void CVKImGuiLayer::Init(GLFWwindow *main_window, IRenderer *renderer) {
  ...
  // Create Descriptor Pool
  {
    ...
    DescriptorPoolCreateInfo.poolSizeCount =
        (uint32_t)IM_ARRAYSIZE(DescriptorPoolSize);
    DescriptorPoolCreateInfo.pPoolSizes = DescriptorPoolSize;
    auto ImGuiDescriptorPoolResult =
        GetVKRenderer()->Device.createDescriptorPool(DescriptorPoolCreateInfo);
...
void CVKImGuiLayer::DrawFrame() { CImGuiLayer::DrawFrame(); }
...
void CVKImGuiLayer::RenderFrame() {
  ...
  RenderPassBeginInfo.renderArea =
      vk::Rect2D(vk::Offset2D(), GetVKRenderer()->SwapchainExtent);
  RenderPassBeginInfo.clearValueCount = 1;
  RenderPassBeginInfo.pClearValues = &clear_color;

  RenderFrame.CommandBuffer.beginRenderPass(RenderPassBeginInfo,
                                            vk::SubpassContents::eInline);
  ImGui::Render();
  ImDrawData *DrawData = ImGui::GetDrawData();
  const bool bIsMinimized =
...
void CVKImGuiLayer::CreateRenderPass() {
  vk::AttachmentDescription Attachments[] = {vk::AttachmentDescription(
      ...
      GetVKRenderer()->SwapchainSurfaceFormat.format,
  ...
  vk::AttachmentReference AttachmentRefs[] = {
      vk::AttachmentReference{0, vk::ImageLayout::eColorAttachmentOptimal}};

  vk::SubpassDescription Subpasses[] = {vk::SubpassDescription(
      vk::SubpassDescriptionFlags(), vk::PipelineBindPoint::eGraphics, nullptr,
      AttachmentRefs, nullptr, nullptr, nullptr)};
  ...
  auto ImGuiRenderPassResult =
      GetVKRenderer()->Device.createRenderPass(RenderPassCreateInfo);
...
void CVKImGuiLayer::CreateFramebuffers() {
  ...
  for (size_t i = 0; i < GetVKRenderer()->MaxFramesInFlight; i++) {
    vk::ImageView Attachments[] = {
        GetVKRenderer()->RenderFrames[i].SwapchainImageView};
    ...
    auto ImGuiFramebufferResult =
        GetVKRenderer()->Device.createFramebuffer(FramebufferCreateInfo);
...
void CVKImGuiLayer::AllocateCommandBuffers() {
  for (size_t i = 0; i < GetVKRenderer()->MaxFramesInFlight; i++) {
    ...
    auto CommandBuffersResult = GetVKRenderer()->Device.allocateCommandBuffers(
